...
def get_arguments():
    ...
    parser.add_argument("--inputDir", "-i", help="Input folder name [default: ./reconstruction_global]")
    parser.add_argument("--outDir", "-o", help="Output folder containing the bootstrapped results")
#   parser.add_argument("--ref", "-r", help="Input NCBI accession number for the reference genome")
    parser.add_argument("--group", "-g", help="Group by text file [default: result/group.txt]")
...
class Block(object):
    ...
    def calculateDistance(self,geneBlock):
        ...
        # remove gene that is not in intersection
...
if __name__ == "__main__":
    ...
    for operon in sorted(res):
        if "mapping" in operon:
            ...
            print ("Bootstrapping operon: ",operonName)
            ...
            for node in tree.iter_descendants("postorder"):
                if not node.is_leaf():
                    ...
                    for candidate in sample:
                        ...
                        sampleTree.write(format=2, outfile=dataOutfile,features=['name','initial','gene_block','deletion','duplication','split','modified'])
                        visualOutfile=visualization+"/"+operonName+"_"+str(count)
                        cmd11='./show_bootstrap.py -i {} -g {} -o {} -m {}'.format(dataOutfile,group,visualOutfile,mapping)
                        os.system(cmd11)
