...
'''@function: Reconstruct the newick tree file with gene block info for inner 
   ...
   @input   : tree in nwk format,and a dictionary between species name and gene block for leaf
    ...
    for node in tree.traverse('postorder'):
        if not node.is_leaf():
            if node.node_type == 'GG':
                ...
                # run the GG function
                if split1 == 0:
                    mytuple = findSetInitial_GG(genome_list[0], genome_list[1], split1, split2)
                else:
                    mytuple = findSetInitial_GG(genome_list[1], genome_list[0], split2, split1)
            ...
            if node.node_type == 'SG':            
                for children in node.get_children():
                    if children.is_leaf():
                        ...
                        initial_tuple = (children.initial, children.elementCount, children.count,
                                        children.dup,children.deletion, children.duplication,
                                        children.split)
            ...
            if node.node_type == 'SS': 
                ...
                tuple1 = (genome_list[0].initial, genome_list[0].elementCount, genome_list[0].count,
                          genome_list[0].dup, genome_list[0].deletion, genome_list[0].duplication,
                          genome_list[0].split)
                tuple2 = (genome_list[1].initial, genome_list[1].elementCount, genome_list[1].count,
                          genome_list[1].dup, genome_list[1].deletion, genome_list[1].duplication,
                          genome_list[1].split)
                # run the SS function
                mytuple = findSetInitial_SS(tuple1, tuple2)
            # at the end, from mytuple, insert the info into the internal node
            node.add_features(initial=mytuple[0], elementCount=mytuple[1], count=mytuple[2],
                              dup=mytuple[3], deletion=mytuple[4], duplication=mytuple[5],
                              split=mytuple[6])            
    return tree
...
if __name__ == "__main__":
    ...
    for r in res:
        ...
        if method.lower() =='local':
            ...
            tree.write(format=2, outfile=outputsession+'/'+f, 
                       features=['name','initial','gene_block','deletion','duplication','split'])
